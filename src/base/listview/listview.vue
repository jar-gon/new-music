<template>
  <scroll class="listview" :data="data" :listenScroll="listenScroll" :probeType="probeType" @scroll="scroll" ref="listview">
    ...
    <div class="list-shortcut" @touchstart.stop.prevent="onShortcutTouchStart" @touchmove.stop.prevent="onShortcutTouchMove">
      <ul>
        <li class="item" v-for="(item, index) in shortcurList" :class="{current: currentIndex === index}" :key="index" :data-index="index">{{item}}</li>
      </ul>
    </div>
  ...
  watch: {
    ...
    scrollY(newY) {
      ...
      for (let i = 0; i < listHeight.length - 2; i++) {
        ...
        if (-newY >= listHeight[i] && -newY < height2) {
          this.currentIndex = i
    ...
    diff(newVal) {
      let fixedTop = (newVal < TITLE_HEIGHT && newVal > 0) ? newVal - TITLE_HEIGHT : 0
